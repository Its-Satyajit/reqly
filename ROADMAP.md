...
| **08** | Search & Palette polish | Search 67–68, Palette + Keyboard | `paletteProviders.ts:19` (`navViews` 16 + `import`/`export`/`theme-*` `5553cd19`) + `CommandPalette` grouping/recent + `useKeyboardMap` `⌘K/⌘1-8` | `[x]` — grouped by `Navigation`/`Theme`/`Environment`/`Collection`/`History`, recent 5 via `localStorage`, directional empty (“Try …”), `⌘K`/`↵`/`Esc` hints |
