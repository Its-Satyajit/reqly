...
13. **OAuth 2.0 / auth leftovers** — [Spec #60](https://github.com/Its-Satyajit/reqly/issues/60) + tickets [#61–#65](https://github.com/Its-Satyajit/reqly/issues/61): device flow (RFC 8628) with `reqly auth login --flow device` — ✅ shipped (T1); OS-keychain token backend (`secrets.Store` drop-in), custom redirect schemes, desktop auth UI — in progress (T2–T5); Password/ROPC stays deferred per OAuth 2.1
