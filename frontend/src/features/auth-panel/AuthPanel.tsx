...
export function AuthPanel() {
  const { status, loading, error, refresh, login, logout, clearError } = useAuthStore()
  const [flow, setFlow] = useState<'authorization_code' | 'device_code'>('authorization_code')
    ...
    <div className="flex flex-col gap-3 px-2 pt-4">
          ...
          {tokens.map((tok, i) => (
            ...
            >
              ...
              <p className="text-muted-foreground">
                Expires {formatExpiry(tok.expiry)} · {tok.hasRefresh ? 'refresh token' : 'no refresh token'} ·{' '}
                <span className={tok.state === 'expired' ? 'text-destructive' : undefined}>
                  {tok.state}
                </span>
              </p>
            </li>
      ...
      >
        <label className="flex flex-col gap-1">
          ...
          <select
            value={flow}
            onChange={(e) => setFlow(e.target.value as 'authorization_code' | 'device_code')}
            className="rounded-md border border-input bg-background px-2 py-1 text-xs text-foreground"
          >
            <option value="authorization_code">Browser (authorization code + PKCE)</option>
            <option value="device_code">Device code (approve on any device)</option>
          </select>
        </label>

        <label className="flex flex-col gap-1">
          <span className="text-xs text-muted-foreground">OAuth 2.0 config (JSON)</span>
          <textarea
            value={configText}
