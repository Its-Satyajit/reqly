  ...
  const onSave = async () => {
    ...
    for (const row of rows) {
      if (row.removing) {
        ...
        if (row.value === '') {
          setError(`Secret "${row.name}" needs a value before it can be saved.`)
          return
        }
      ...
      <div className="flex flex-col gap-1.5">
        {rows.map((row, i) => (
          <div key={row.name} className="flex items-center gap-1.5">
            <span className="w-40 shrink-0 truncate font-mono text-xs">{row.name}</span>
            <input
              type={row.revealed ? 'text' : 'password'}
              value={row.value}
              onChange={(e) =>
                setRow(i, { value: e.target.value, touched: e.target.value !== '' })
              }
              disabled={row.removing}
              placeholder={row.removing ? 'will be removed' : 'unchanged — type to replace'}
              spellCheck={false}
            ...
            <Button
              variant="ghost"
              size="icon-sm"
              aria-label={row.revealed ? `mask secret ${row.name}` : `reveal secret ${row.name}`}
            ...
            <Button
              variant="ghost"
              size="icon-sm"
              aria-label={row.removing ? `keep secret ${row.name}` : `remove secret ${row.name}`}
              onClick={() => setRow(i, { removing: !row.removing, value: '', touched: false })}
            >
