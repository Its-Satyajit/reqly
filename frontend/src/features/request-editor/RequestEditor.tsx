...
import { type BodyType, bodyTypes } from "../../lib/body";
...
export function RequestEditor() {
	...
	const loading = useRequestStore((s) =>
		activeTabId ? s.responses[activeTabId]?.loading : false,
	);
	...
	const handleSend = () => {
		void send(activeTabId, {
			...
			headers: sentRows(draft.headers).map(({ key, value }) => ({
				key,
				value,
			})),
	...
	const patch = (p: Partial<Parameters<typeof updateDraft>[1]>) =>
		updateDraft(activeTabId, p);
		...
		<div className="flex h-full flex-col">
			<div className="flex items-center gap-2 p-2">
				...
				<span
					title={
						meta?.env
							? "Environment pinned by the request file"
							: "Environment from the app header"
					}
...
function VariablesView({
	...
	return (
		<div className="flex flex-col gap-2">
			...
			<p className="text-xs text-muted-foreground">
				Environment: {env ?? headerEnv ?? "none"} (
				{env
					? "pinned by file"
					: headerEnv
						? "from app header"
