...
SQLite will be used only where structured local metadata provides a meaningful advantage. Access is via `modernc.org/sqlite` (pure Go, no CGO) with **sqlc**-generated typed queries (`internal/history/db`; SQL sources in-repo, regenerate with `sqlc generate` — see ADR 0027). GUI-side search is client-side Fuse.js; FTS5 remains the CLI index behind a query sanitizer.
