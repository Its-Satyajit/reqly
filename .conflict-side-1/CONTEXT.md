...
### Workflow
A Git-native, sequential multi-step API execution unit defined in YAML (`name`, `variables`, `steps[]`). Each step carries a `request.Request` plus optional `condition` (Goja JS boolean) and `extract` (response-JSON key → variable). Executed by `WorkflowExecutor` with shared `variables.Set` propagation so a prior step's `extract` feeds the next step's `{{var}}` interpolation.

### Workflow Step
One entry in `Workflow.steps`: `id`, `name`, `request`, optional `condition` (e.g. `reqly.getVariable('token') !== ''`), and optional `extract` map (`varName → jsonKey`). The condition is evaluated via Goja with `reqly.getVariable` bound to the live `variables.Set` (Resolve precedence); false skips the step, error or true runs it. `extract` reads the JSON response body (`map[string]any`, top-level keys) into `ScopeGlobal` and the report's `extractedVars`.

### Workflow Report
The aggregate result of a workflow run: `workflowName`, `passed` (all executed steps <400 and no transport error), `duration`, ordered `steps: StepResult[]`, and `extractedVars` (all variables extracted across steps). Produced by `WorkflowExecutor.Execute` and consumed by CLI stdout and Goja `reqly.workflow.run`.

### Step Result
The per-step outcome inside a `WorkflowReport`: `name`, `requestPath` (= step `id`), `passed` (status <400 and no error), `requestError` (string, empty on success), `response` (`*response.Response`), and `logs` (one line `METHOD URL -> status (ms)` or error). Skipped steps never appear in the report.

### Workflow Executor
The `internal/workflow.WorkflowExecutor` seam: `NewWorkflowExecutor(*request.Client)` (nil → default client) and `Execute(ctx, *Workflow, WorkflowOptions) (*WorkflowReport, error)`. Owns variable layering (`Workflow.variables` → `ScopeGlobal`, `WorkflowOptions.EnvironmentVars` → `ScopeEnvironment`, `extract` → `ScopeGlobal`), Goja condition evaluation, `variables.Interpolate` for URL/headers/query/body (cloned slices, ignore-interpolation errors), and `request.Client.Execute` with shared `variables.Set`.

### Workflow Options
Per-run options for `WorkflowExecutor.Execute`: `EnvironmentVars map[string]string` merged into the `variables.Set` at `ScopeEnvironment` before execution, so a caller can inject environment-scoped variables without mutating the stored workflow file.

### Workflow Condition
A Goja-evaluated JS boolean expression gating a step. The sandbox exposes only `reqly.getVariable(name)` (Resolve precedence, returns `""` when missing). Evaluation is one-shot `vm.RunString(condition)`; `err == nil && !val.ToBoolean()` → skip, otherwise run. No access to request/response; script errors never skip — the step runs.

### Workflow Variable Extraction
Post-response extraction of top-level JSON keys into the workflow's variable store (`extract: {token: token}`). Only applied when `resp.Body` is valid JSON object; each `jsonKey` existence is checked via `parsed[jsonKey]` and stringified with `fmt.Sprintf("%v", val)`. Extracted variables are set at `ScopeGlobal` and returned in `WorkflowReport.ExtractedVars` for CLI `Extracted Variables:` printing and Goja `report.extractedVars`.

### Automation
A Git-native, self-hosted schedule for a Workflow. Stored as YAML (e.g. `.reqly/automation/<name>.yaml` or `automation.yaml`) with `name`, `workflow` (full `workflow.Workflow` struct), `interval` (Go duration string, e.g. `"10s"`, `"5m"`; `"0"` or empty means run once), optional `enabled` (nil = true, `false` disables), and optional `maxRuns` (0 = infinite until context cancel). Validated via `Automation.Validate` (`name` + `workflow.steps` required, interval must parse, `maxRuns >=0`).

### Automation Scheduler
The `internal/automation.Scheduler` seam: `NewScheduler(*request.Client)` (nil → default) and `Run(ctx, *Automation, func(*workflow.WorkflowReport)) error`. Immediate first run, then `time.NewTicker(interval)` loop; interval 0 runs once. Respects `Enabled` (error when disabled), `MaxRuns` cap, and `ctx.Done()` (returns `ctx.Err()`). Reuses `workflow.NewWorkflowExecutor` so every tick shares variable extraction, condition, and interpolation.

### Automation CLI
`reqly automation run <automation.yaml> [--once] [--interval <dur>] [--max-runs N]` — loads automation YAML, applies flag overrides (`--once` → `interval="0"`, `--interval`/`--max-runs` replace file values), validates, then `Scheduler.Run` with `onReport` printing `[#] Automation "name" workflow "wf": PASSED (N steps, duration)` plus per-step lines. Exit 0 on `context.Canceled`, else error.

### Automation Desktop
Desktop parity for automation: `AppService.AutomationRun(yamlStr string) (*workflow.WorkflowReport, error)` — YAML-unmarshals, forces `interval="0"` to run once (desktop does not schedule background ticks), validates, then `Scheduler.Run` with single-report capture. Exposed to Wails as `AutomationRun` binding, mirroring `WorkflowRun`.
...
### Audit Entry
One audit trail record (`Entry{ID,Timestamp,Actor,Action,Resource,Details}`) validated for 11 actions, stored as `.reqly/audit.log` JSONL 0600 via `internal/audit.Store`.

### Policy
Local organization policy (`.reqly/policy.yaml` 0600, `Policy{RequireAudit,MaxWorkflowSteps,AllowedActions}`) enforced via `internal/policy.Enforce`.

### RBAC
Local RBAC (`.reqly/rbac.yaml` 0600, `RBAC{Roles,UserRoles}`) with `admin` (`*`), `editor` (6 perms), `viewer` (2 perms), enforced via `internal/rbac.Can`.

### Vault Store
HashiCorp Vault KV v2 store (`VaultStore{Addr,Token,Mount,Prefix}`) via `X-Vault-Token`, `REQLY_TOKEN_STORE=vault`.

### SSO Config
OIDC config (`Config{Issuer,ClientID,JWKSURL,AllowedGroups}`) validated via `internal/sso.ValidateToken` (HMAC).

### SCIM User
SCIM 2.0 user (`User{ID,UserName,Email,Groups,Active}`) via `internal/scim.Store`.

### Shared Workspace
Git-native shared workspace (`.reqly/collab.yaml` 0600, `SharedWorkspace{Path,Collaborators}`) via `internal/collab`.

### Collab Server
Self-hosted HTTP server (`Server{root,mux}`) with `/health`/`/collab`/`/workspace` via `internal/collab.Server`.

### Integration Pipeline
Full pipeline test (`workflow` + `policy` + `RBAC` + `collab` + `audit` + `httptest`) via `internal/integration`.
